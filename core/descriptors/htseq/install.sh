...
d="$1"
archive="${flight_PACK_store_dir}/spack+htseq.tar.bz2"
...
if [ -e "${archive}" ]; then
    if [ -d "${target_root}"/spack+htseq ]; then
	pack_err "Packaged environment already exists: ${target_root}/spack+htseq"
	exit 1
    else
