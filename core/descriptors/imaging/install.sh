...
d="$1"
archive="${flight_PACK_store_dir}/spack+imaging.tar.bz2"
...
if [ -e "${archive}" ]; then
    if [ -d "${target_root}"/spack+imaging ]; then
	pack_err "Packaged environment already exists: ${target_root}/spack+imaging"
	exit 1
    else
