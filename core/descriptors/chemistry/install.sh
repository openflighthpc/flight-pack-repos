...
d="$1"
archive="${flight_PACK_store_dir}/spack+chemistry.tar.bz2"
...
if [ -e "${archive}" ]; then
    if [ -d "${target_root}"/spack+chemistry ]; then
	pack_err "Packaged environment already exists: ${target_root}/spack+chemistry"
	exit 1
    else
