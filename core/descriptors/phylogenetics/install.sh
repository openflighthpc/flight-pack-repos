...
d="$1"
archive="${flight_PACK_store_dir}/spack+phylogenetics.tar.bz2"
...
if [ -e "${archive}" ]; then
    if [ -d "${target_root}"/spack+phylogenetics ]; then
	pack_err "Packaged environment already exists: ${target_root}/spack+phylogenetics"
	exit 1
    else
